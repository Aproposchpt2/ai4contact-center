...
<Response>
  <Gather input="speech" timeout="${SPEECH_TIMEOUT}" speechTimeout="auto"
          action="${safeString(process.env.FLOWDESK_SITE_URL, 'https://contact-center.aiflowdeskpro.com')}/.netlify/functions/call?step=${PROCESS_STEP}"
          method="POST">
    <Say voice="Polly.Joanna" language="en-US">${escapeXml(greeting)}</Say>
  </Gather>
  <Redirect method="POST">${safeString(process.env.FLOWDESK_SITE_URL, 'https://contact-center.aiflowdeskpro.com')}/.netlify/functions/call?step=${FALLBACK_STEP}</Redirect>
</Response>`;
}
